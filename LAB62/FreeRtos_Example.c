...
// SETTING UP UART0 TO ECHO ON THE TERMINAL
void ConfigureUART(uint32_t system_clock, uint32_t baud_rate)
{
    // SETTING UP THE UART0 PERIPHERAL CONTROLL
    SysCtlPeripheralEnable(SYSCTL_PERIPH_UART0);

    // SETTING UP PORTA PIN0 AND PIN1 FOR UART0
...
// FUNCTION THAT WILL RECEIVE TEXT
...
//THIS ASCII VALUE COMBINATION UPDATE THE TEXT ON THE TERMINAL AND CLEAR THE CURSOR
...
// THIS FUNCTION CREATE AND INTEGER VALUE WHEN THE LEFT BUTTON PJ0 IS PRESSED
...
void vTaskButton(void* pvInput){
    ...
    while(1){
     ...
     // WE WAIT FOR A SMALL DELAY BEFORE POOLING AGAIN
        vTaskDelay(25);
    }
}
...
void main(void)
{
  ...
  // CONFIQURE AND SETTING UP SYSTEM CLOCK
 ...
 // UPDATE THE DISPLAY
   ...
   // HAND OVER THE PROCESS TO THE SCHEDULER
    vTaskStartScheduler();
